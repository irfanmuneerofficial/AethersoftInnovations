const IoriTheme = IoriTheme || {}
window.ioriTheme = IoriTheme
...
IoriTheme.handleError = (data) => {
    if (typeof data.errors !== 'undefined' && data.errors.length) {
        IoriTheme.handleValidationError(data.errors)
    } else if (typeof data.responseJSON !== 'undefined') {
        if (typeof data.responseJSON.errors !== 'undefined') {
            if (data.status === 422) {
                IoriTheme.handleValidationError(data.responseJSON.errors)
            }
        } else if (typeof data.responseJSON.message !== 'undefined') {
            IoriTheme.showError(data.responseJSON.message)
        } else {
            $.each(data.responseJSON, (index, el) => {
                $.each(el, (key, item) => {
                    IoriTheme.showError(item)
                })
            })
        }
    } else {
        IoriTheme.showError(data.statusText)
    }
}

IoriTheme.handleValidationError = (errors) => {
    let message = ''
...
;(function($) {
   ...
   $(document).ready(function() {
    ...
    function showMonthPrice() {
        const boxPrice = $('.price-plan').find('.box-price')
        const priceMonths = boxPrice.find('.month_price')
        const priceYears = boxPrice.find('.year_price')

        priceMonths.each((i, e) => {
            $(e).show()
        })
